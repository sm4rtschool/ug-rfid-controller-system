...
class Controller extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model('Controllermodel', 'controller');
	...
	public function index()
	{
		...
		*/
		...
		$this->load->view('controller/controller', $data);
	...
	public function update_status(){
		...
		$is_success = $this->home->update_status($id_temp_table, $rfid_tag_number, $output, $room_id, $reader_id, $kategori_pergerakan, $keterangan_pergerakan, $lokasi_terakhir, $nama_lokasi_terakhir);
		
		if (!$is_success) {
			$response = array('is_success' => $is_success, 'message' => 'Status has not been updated.');
		} else {
			$response = array('is_success' => $is_success, 'message' => 'Status has been updated.');
		}
		...
		if ($this->input->method() === 'post') {
			...
			if (!empty($data['data'])) {
				...
				foreach ($data['data'] as $item) {
					...
					$flag_alarm = $item['flag_alarm'];
					$ant = $item['ant'];
					$alias_antenna = $item['alias_antenna'];
					$no_sku = $item['no_sku'];

					$result_response = $this->controller->saveData($id, $tid, $epc, $status, $created_time, $description, $category, $flag_alarm, $ant, $alias_antenna, $no_sku);
					$is_success = $result_response['is_success'];
				...
				if ($is_success) {
					echo json_encode([
						'is_success' => true,
						'is_tag_registered' => $result_response['is_tag_registered'],
						'is_exists' => $result_response['is_exists'],
